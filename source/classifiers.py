...
#from source.pulearning import ADOA
from xgboost import XGBClassifier
#from source.pu_learning import pu_learning 
#
#
#def pulearning(X,y):
#    clf2 = XGBClassifier(n_estimators=350, learning_rate=0.25, max_depth=6, n_jobs=-1, random_state=2018)
#    clf1 = RandomForestClassifier(n_estimators=150 ,random_state=2018)
#    a = pu_learning(X ,y ,clf1=clf1 ,clf2=clf2 ,sample_ratio=0.05 ,over_sample=False)
#    return a

#
#def ad(X,y):
#    # xgb = XGBClassifier(n_estimators=150, learning_rate=0.25, max_depth=6, n_jobs=-1, random_state=2018)
#    svm = SVC(C=1.0, kernel='rbf', gamma='auto', probability=True, degree=3, random_state=2020)
#    clf = ADOA( X, y, svm)
#    # y_pred, y_prob = clf.predict()
#    return clf
...
def IsolationForest1(X):
     iforest = IsolationForest(n_estimators=300, contamination=0.04, random_state=2020)
#     clf = iforest.fit(X)
#     return clf.predict() 
     return iforest.fit(X)
...
def labelPropagation(X, y, K):
    return label_propagation.LabelSpreading(kernel='knn', n_neighbors=K, alpha=0.8).fit(X, y)
...
def knn_classify(training_data, labels, test_instance):
    ...
    best_distance = np.inf    #np.inf表示的是一个无限大的数
...
def kde(points, allPoints):
    kernel = KernelDensity(kernel='gaussian', bandwidth=0.8).fit(allPoints)
...
#        elif clf =='ADAO' :
#            return ADAOClassifier(X,y)
        elif clf == 'ad':
            return ad(X,y)
#        elif clf == 'pu':
#            return pulearning(X,y)