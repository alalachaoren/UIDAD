...
def run(dataValues, dataLabels, datasetDescription, isBinaryClassification, isImbalanced, experiments, batches, labeledData, isBatchMode, poolSize, plot_anim, externalResults={}):
    ...
    arrYt = dataLabels[labeledData:]    #取标签数据之后的预测数据标签
    arrYt = [ arrYt[i::batches] for i in range(batches) ]   #将预测标签分块
       ...
       # 字典(Dictionary) items() 函数以列表返回可遍历的(键, 值) 元组数组。
        try:
          ...
          #set() 函数创建一个无序不重复元素集，可进行关系测试，删除重复数据，还可以计算交集、差集、并集等。
            ...
            start = timer()
            #accuracy per step
            algorithmName, accuracies, CoreX, CoreY, arrX, arrY, arrUt, arrYt, arrClf, arrPredicted, arrF1, arrjingdu, arrrecall = e.method.start(
            ...
            print("Average recall:",np.mean(arrrecall))
            print("Average 精度:",np.mean(arrjingdu))
