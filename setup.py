...
def loadKeystroke(path, sep):
    ...
    return dataValues, dataLabels.astype(int), description
def loadkddcup(path,sep):
    description = 'kddcup99 . high dimension.2 class.'
    
    dataValues =pd.read_csv(path+'real'+sep+'kddcup99'+sep+'training.csv',sep=',',header=None)
    dataValues =pd.DataFrame.as_matrix(dataValues)
    dataLabels = dataValues[:, 32]
    dataValues = dataValues[:,4:32]
    dataValues = MinMaxScaler().fit_transform(dataValues)
    dataValues = classifiers.pca(dataValues, 2)
    #prinnt(dataValues)
    #countInstances("kddcup99", dataLabels)
    return dataValues,dataLabels.astype(int),description

def loadiot(path,sep):
    description = 'IoT_trace_duofenlei .11 features .anomaly detection'
    
    dataValues =pd.read_csv(path+'real'+sep+'iot'+sep+'smarthome_quan.csv',sep=',',header=None)
    dataValues = pd.DataFrame.as_matrix(dataValues)
    dataLabels = dataValues[:, 11]
    dataValues = dataValues[:, 0:11]
    dataValues = MinMaxScaler().fit_transform(dataValues)
    # dataValues = classifiers.pca(dataValues, 2)
    #countInstances("IoT_trace_duofenlei", dataLabels)
    return dataValues,dataLabels.astype(int),description

def loadkdd(path,sep):
    description = 'kddcup99 完整datasets multi class'

    dataValues =pd.read_csv(path+'real'+sep+'kddcup99'+sep+'test.csv',sep=',',header=None)
    dataValues = pd.DataFrame.as_matrix(dataValues)
    dataLabels = dataValues[:,41]
    dataValues = dataValues[:,4:41]
    dataValues = MinMaxScaler().fit_transform(dataValues)
    dataValues = classifiers.pca(dataValues, 2)
    return dataValues,dataLabels.astype(int),description
...
def loadElecData(path, sep):
    ...
    dataValues = pd.DataFrame.as_matrix(dataValues)
    dataLabels = dataValues[:, 7]#取第七列的所有行值
    dataValues = dataValues[:, 2:7]#X[:,  m:n]，即取所有数据的第m到n-1列数据，含左不含右
    #print(dataValues)
    #countInstances("elecdata", dataLabels)
    return dataValues, dataLabels.astype(int), description

#def loadmainSimulationAccessTraces(path, sep):
#    description = 'Electricity data. 12 features. 2 classes.'
#    dataValues = pd.read_csv(path+'real'+sep+'elecdata'+sep+'electricity_dataset.csv',sep = ",", header=None)
#    dataValues = pd.DataFrame.as_matrix(dataValues)
